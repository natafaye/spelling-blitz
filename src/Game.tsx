...
import { GAME_LENGTH_IN_SECONDS, MAX_GAME_HISTORY_LENGTH } from "./shared/constants";
...
export default function Game() {
    ...
    const [gameIdHistory, setGameIdHistory] = useLocalStorageState<string[]>("gameIdHistory", [])
    ...
    const joinGame = (id: string) => {
        setGameId(id)
        setGameIdHistory([id, ...gameIdHistory]
            .filter((item, index, array) => array.indexOf(item) === index)
            .slice(0, MAX_GAME_HISTORY_LENGTH)
        )
    }

    // Leave the game
    const leaveGame = () => {
        // Don't delete games (may need to turn on later if database is filling up)
    ...
    if (!gameId)
        return <Start playerId={playerId} gameIdHistory={gameIdHistory} onJoin={joinGame} />
